...
**日期**：2026-05-31  
**文件**：`liva.py`，函数 `run_stage_danger()`  
**范围**：最小改动，仅修 `liva.py`，不动 Java 脚本、config.ini、infer 阶段
...
2. **Bug 2**：`parsed_sources` 只取 LLM 识别结果（`config.LivaConfig.source_point`），
...
**位置**：`liva.py` `run_stage_danger()`，现行第 268–273 行附近
...
if len(config.LivaConfig.sink_point) != 0:
    ...
    for s in config_sinks:
        if s not in config.LivaConfig.sink_point:
...
**位置**：`liva.py` `run_stage_danger()`，现行第 260–264 行附近
